...
import { socket, connectSocket, saveGameData, saveBrowserSession, getBrowserSession } from "../../services/websocket/socketService";
...
export default function CharacterSelection() {
  ...
  // 🔑 Verificar sesión del navegador al cargar
  React.useEffect(() => {
    ...
    if (!currentPin || !currentUsername) {
      console.log('❌ Faltan datos de juego, redirigiendo...');
      navigate('/');
      return;
    }
    
    // Verificar si ya hay una sesión activa en este navegador
    const browserSession = getBrowserSession();
    if (browserSession && browserSession.pin === currentPin) {
      console.log('🔄 Sesión del navegador detectada');
      console.log('   Usuario existente:', browserSession.username);
      console.log('   Usuario actual:', currentUsername);
      
      // Si es el mismo usuario, restaurar y continuar
      if (browserSession.username === currentUsername) {
        console.log('✅ Mismo usuario, restaurando sesión...');
        setSelectedCharacter(browserSession.character);
        // Auto-unirse con el personaje guardado
        handleJoinGameWithCharacter(browserSession.character);
      }
      // Multicuentas permitidas - no bloqueamos
    }
  }, [navigate]);
  
  const handleJoinGameWithCharacter = (character) => {
    const pin = localStorage.getItem("gamePin");
    const username = localStorage.getItem("tempUsername");
    
    if (!character || !pin || !username) return;
    ...
    socket.emit("join-game", {
      ...
      setLoading(false);
      if (response.success) {
        console.log('✅ Unido al juego con socket.id:', response.socketId);
        
        // Guardar sesión del navegador
        saveBrowserSession({
          pin,
          username,
          character: {
  ...
  const handleConfirmSelection = () => {
    ...
    handleJoinGameWithCharacter(selectedCharacter);
  };
  
  // Función auxiliar para manejar el join (no exportada, solo interna)
  const _oldHandleJoin = () => {
    const pin = localStorage.getItem("gamePin");
    const username = localStorage.getItem("tempUsername");
